...
const getBaseInfoByKey = (key, reqandresp, typingDir) => {
  ...
  const keyUrl = keys[1] || "";
  const fileName = keyUrl.substring(keyUrl.lastIndexOf("/"), keyUrl.length);
...
const isRestfulPath = function (url) {
  return url.match(restfulReg);
};
const generateApi = (data, apiImportMoudle) => {
  const content = Object.keys(data).map((orginalKey) => {
    const orginalKeyList = orginalKey.split(" ").filter((item) => item != "");
    const key = orginalKeyList[1];
    const method = orginalKeyList[0].toLowerCase();
    const params = [];

    key.split("/").forEach((item) => {
      if (isRestfulPath(item)) {
        const name = item.replace(restfulReg, "");
        params.push(name);
      }
    });
    const name = orginalKey
    ...
    if (params.length > 0) {
      return `export const  ${methodName}= (restfulParams: {${params.map(
        (item) => `${item}:string;`
      )}}) => net.${method}Api('${key}', {
        restful: restfulParams,
      });`;
    } else {
      return `export const ${methodName} = net.${method}Api(
        "${key}"
      );`;
    }
  });
