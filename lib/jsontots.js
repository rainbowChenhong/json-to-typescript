...
const apiImportMoudle = config.apiImportMoudle || "";
...
const start = () => {
  ...
  generateApi(requestRouteData, apiImportMoudle);
};
...
const generateApi = (data, apiImportMoudle) => {
  const content = utils.generateApi(data, apiImportMoudle);
  const apiPath = path.join(currentPath, typingDir);
  if (!fs.existsSync(apiPath)) {
    fs.mkdirSync(apiPath, {recursive: true});
  }
  fs.writeFile(`${apiPath}/api.ts`, content, (err) => {
    if (err) {
      console.error(err);
      return;
    }
